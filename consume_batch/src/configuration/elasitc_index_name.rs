...
pub static CONSUME_TYPE_SETTINGS: once_lazy<String> = once_lazy::new(|| {
    dotenv().ok();
    env::var("CONSUME_TYPE_SETTINGS").expect("[ENV file read Error] 'CONSUME_TYPE_SETTINGS' must be set")
});
...
pub static CONSUME_DETAIL_SETTINGS: once_lazy<String> = once_lazy::new(|| {
    dotenv().ok();
    env::var("CONSUME_DETAIL_SETTINGS").expect("[ENV file read Error] 'CONSUME_DETAIL_SETTINGS' must be set")
});
